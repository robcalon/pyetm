...
def _handle_nans(worksheet: Worksheet,
    row: int, col: int, number: float, cell_format=None) -> Literal[-1, 0]:
    ...
    if isinstance(frame.columns, pd.MultiIndex):
        ...
        for col_num, col_data in enumerate(frame.columns.values):
            for row_num, cell_data in enumerate(col_data):
                worksheet.write(
                    row_num, col_num + skipcolumns, cell_data, cell_format)
   ...
   # set column widths
    ...
    if index is True:
        _write_index(worksheet=worksheet,
            ...
            column_width=column_width, cell_format=cell_format)
